...
BRTC::BRTC(QObject *parent) : QObject(parent)
{
...
BRTC::~BRTC()
{
    ...
    qDebug()<< "logoutRoom OK.";
    g_BrtcClient->Destory();
    g_BrtcClient = nullptr;
    qDebug()<< "Destory OK.\n";

    if (dlclose(handle) != 0) {
        qDebug()<<  "Could not close plugin: %s\n";//, dlerror());
        //return 1;
    }
}
...
void  BRTC::call_brtc(QString id, QString room)
{
    quint8 ret=0;
    handle = dlopen("libbaidurtc.so", RTLD_LAZY);
    if (handle == NULL) {
    ...
    if(ret==0){
        ...
        g_BrtcClient->setAppID( id.toUtf8() );// argv[1]$(APPID_GET_FROM_BAIDU)  "appmajfz6bn7x4s"
        ...
        g_BrtcClient->loginRoom(room.toUtf8(),uid.c_str(),"BRTC Linux SDK push demo","token");
    }
}
